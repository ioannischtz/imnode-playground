...
inline bool RectangleOverlapsLineSegment(const ImRect &rect, const ImVec2 &p1,
  ...
  const int corner_signs[4] = {
      ...
      Sign(
          EvalImplicitLineEq(p1, p2, ImVec2(flip_rect.Max.x, flip_rect.Min.y))),
      Sign(
          EvalImplicitLineEq(p1, p2, ImVec2(flip_rect.Min.x, flip_rect.Max.y))),
...
void ImDrawListGrowChannels(ImDrawList *draw_list, const int num_channels) {
  ...
  // modifications to allow nested splits. The main modification is that we only
  // create new ImDrawChannel instances after splitter._Count, instead of over
  // the whole splitter._Channels array like the regular
...
bool MouseInCanvas() {
  // This flag should be true either when hovering or clicking something in the
  // canvas.
...
void TranslateSelectedNodes(ImNodesEditorContext &editor) {
  if (GImNodes->LeftMouseDragging) {
    // If we have grid snap enabled, don't start moving nodes until we've moved
    // the mouse slightly
...
bool ShouldLinkSnapToPin(const ImNodesEditorContext &editor,
  ...
  // The link to be created must not be a duplicate, unless it is the link which
  // was created on snap. In that case we want to snap, since we want it to
  // appear visually as if the created link remains snapped to the pin.
...
void ClickInteractionUpdate(ImNodesEditorContext &editor) {
  ...
  case ImNodesClickInteractionType_LinkCreation: {
    ...
    const bool should_snap =
        GImNodes->HoveredPinIdx.HasValue() &&
        ShouldLinkSnapToPin(editor, start_pin, GImNodes->HoveredPinIdx.Value(),
                            maybe_duplicate_link_idx);

    // If we created on snap and the hovered pin is empty or changed, then we
    // need signal that the link's state has changed.
    ...
    // Detach the link that was created by this link event if it's no longer in
    // snap range
    if (snapping_pin_changed && GImNodes->SnapLinkIdx.HasValue()) {
      BeginLinkDetach(editor, GImNodes->SnapLinkIdx.Value(),
...
void ResolveOccludedPins(const ImNodesEditorContext &editor,
  ...
  for (int depth_idx = 0; depth_idx < (depth_stack.Size - 1); ++depth_idx) {
    ...
    // Iterate over the rest of the depth stack to find nodes overlapping the
    // pins
...
ImOptionalIndex ResolveHoveredPin(const ImObjectPool<ImPinData> &pins,
  ...
  for (int idx = 0; idx < pins.Pool.Size; ++idx) {
    ...
    // TODO: GImNodes->Style.PinHoverRadius needs to be copied into pin data and
    // the pin-local value used here. This is no longer called in
...
ImOptionalIndex ResolveHoveredLink(const ImObjectPool<ImLinkData> &links,
  ...
  // 1. The link is within hover distance to the mouse. The closest such link is
  // selected as being hovered over.
  // 2. If the link is connected to the currently hovered pin.
  //
  // The latter is a requirement for link detaching with drag click to work, as
  // both a link and pin are required to be hovered over for the feature to
  // work.
  ...
  for (int idx = 0; idx < links.Pool.Size; ++idx) {
    ...
    // If there is a hovered pin links can only be considered hovered if they
    // use that pin
    ...
    // TODO: the calculated CubicBeziers could be cached since we generate them
    // again when rendering the links
    ...
    // The distance test
    {
      ...
      // First, do a simple bounding box test against the box containing the
      // link to see whether calculating the distance to the link is worth
      // doing.
      if (link_rect.Contains(GImNodes->MousePos)) {
        ...
        // TODO: GImNodes->Style.LinkHoverDistance could be also copied into
        // ImLinkData, since we're not calling this function in the same scope
        // as ImNodes::Link(). The rendered/detected link might have a different
        // hover distance than what the user had specified when calling Link()
...
void DrawPinShape(const ImVec2 &pin_pos, const ImPinData &pin,
  ...
  case ImNodesPinShape_CircleFilled: {
    GImNodes->CanvasDrawList->AddCircleFilled(pin_pos,
                                              GImNodes->Style.PinCircleRadius,
                                              pin_color, CIRCLE_NUM_SEGMENTS);
  } break;
...
void DrawNode(ImNodesEditorContext &editor, const int node_idx) {
  ...
  {
    // node base
    GImNodes->CanvasDrawList->AddRectFilled(node.Rect.Min, node.Rect.Max,
                                            node_background,
...
static void MiniMapUpdate() {
  ...
  GImNodes->CanvasDrawList->AddRectFilled(mini_map_rect.Min, mini_map_rect.Max,
                                          mini_map_background);
...
void EndNodeEditor() {
  ...
  // Detect ImGui interaction first, because it blocks interaction with the rest
  // of the UI
  ...
  // hierarchical fashion, because a UI element being hovered excludes any other
  // as being hovered over.

  // Don't do hovering detection for nodes/links/pins when interacting with the
  // mini-map, since its an *overlay* with its own interaction behavior and must
  // have precedence during mouse interaction.
  ...
  if ((editor.ClickInteraction.Type == ImNodesClickInteractionType_None ||
    ...
    // We don't check for hovered pins here, because if we want to detach a link
    // by clicking and dragging, we need to have both a link and pin hovered.
  ...
  // In order to render the links underneath the nodes, we want to first select
  // the bottom draw channel.
...
static const ImNodesStyleVarInfo GStyleVarInfo[] = {
    ...
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, NodeCornerRounding)},
    // ImNodesStyleVar_NodePadding
    {ImGuiDataType_Float, 2, (ImU32)IM_OFFSETOF(ImNodesStyle, NodePadding)},
    // ImNodesStyleVar_NodeBorderThickness
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, NodeBorderThickness)},
    // ImNodesStyleVar_LinkThickness
    {ImGuiDataType_Float, 1, (ImU32)IM_OFFSETOF(ImNodesStyle, LinkThickness)},
    // ImNodesStyleVar_LinkLineSegmentsPerLength
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, LinkLineSegmentsPerLength)},
    // ImNodesStyleVar_LinkHoverDistance
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, LinkHoverDistance)},
    // ImNodesStyleVar_PinCircleRadius
    {ImGuiDataType_Float, 1, (ImU32)IM_OFFSETOF(ImNodesStyle, PinCircleRadius)},
    // ImNodesStyleVar_PinQuadSideLength
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, PinQuadSideLength)},
    // ImNodesStyleVar_PinTriangleSideLength
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, PinTriangleSideLength)},
    // ImNodesStyleVar_PinLineThickness
    {ImGuiDataType_Float, 1,
     (ImU32)IM_OFFSETOF(ImNodesStyle, PinLineThickness)},
...
bool IsLinkDropped(int *const started_at_id,
  ...
  const bool link_dropped =
      ...
      (including_detached_links || editor.ClickInteraction.LinkCreation.Type !=
                                       ImNodesLinkCreationType_FromDetach);
